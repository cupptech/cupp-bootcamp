...
angular.module('starter', ['ionic', 'btford.socket-io', 'ngSanitize', 'ngCordova', 'ngCordovaOauth'])
...
.controller('LoginController', function($scope, $state, $cordovaOauth, $http){ // 
  ...
  $scope.join = function(nickname){
    if (nickname) {
      $state.go('chat', { data: {
        nickname: nickname,
        displayPicture: 'http://www.smartcallcentersolutions.com/images/profile_icon.png'
      }});
    }
  };
  ...
  $scope.loginWithFacebook = function(){
    ...
    $cordovaOauth.facebook('625900327559647', ['email']).then(function(result){
      alert(result.access_token);
      $http.get('https://graph.facebook.com/v2.6/me?fields=id,name,picture&access_token=' 
        + result.access_token)
...
.controller('ChatController', function($scope, $stateParams, Socket, $sce, 
      $ionicScrollDelegate, $timeout, $cordovaMedia){
  ...
  var fillWithEmoticons = function(message){ //(y)
